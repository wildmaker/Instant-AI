# Native models used by Instant-AI

This folder contains Native ONNX or local models used by the application to run various tasks like Whisper for audio to text transcription.

Currently, Instant-AI uses this folder for the following parts of the application.

## Embedding models
- The [OnnxEmbedder](../../utils/EmbeddingEngines/native/index.js) class uses this folder to store the local embedding model files.

## Audio transcription via the Document processor
Instant-AI allows you to upload various audio and video formats as source documents. In all cases the audio tracks will be transcribed by a locally running ONNX model **whisper-small** built by [Xenova](https://github.com/xenova) specifically for WebAssembly use, but works perfectly fine for Node and the server, although it does benefit from GPU compute for faster transcription.
